...
def create_options_parser():
    usage = "Usage: %prog (-f) (-s <addr>) (-c <num>) (-A <arm|arm64>) (-M <arm|thumb>)"
    ...
    parser.add_option('-c', '--count', dest='count', help='decode instruction count (default: 7)', default=7)
...
def real_disassemble(debugger, start_addr, disasm_count, disasm_arch, disasm_mode):
    ...
    for i in range(int(disasm_count)):
        # read bytes
        error = lldb.SBError()
        bytes = process.ReadMemory(start_addr, 4, error)
        ...
        start_addr = insn.address + insn.size
